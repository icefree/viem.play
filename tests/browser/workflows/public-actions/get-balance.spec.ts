  ...
  test('完整工作流: 通过 API 创建节点 → 连线 → 获取余额', async ({ page }) => {
    ...
    const createResult = await page.evaluate(() => {
      ...
      try {
        ...
        clientNode.pos = [350, 180]
        graph.add(clientNode)
        
        // 创建 Constant 节点提供测试地址
        const addressNode = LiteGraph.createNode('basic/const')
        if (!addressNode) return { success: false, error: 'Failed to create Constant node' }
        addressNode.pos = [350, 350]
        // 使用 Vitalik 的地址作为测试地址
        addressNode.setValue = '0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045'
        ...
        balanceNode.pos = [600, 250]
        graph.add(balanceNode)
        
        return { 
          success: true, 
          nodeCount: graph._nodes?.length || 0,
          nodes: [chainNode.title, httpNode.title, clientNode.title, addressNode.title, balanceNode.title]
        }
      } catch (e) {
    ...
    const connectResult = await page.evaluate(() => {
      ...
      // 按位置排序节点 (从左到右，从上到下)
      const sortedNodes = [...nodes].sort((a: { pos: number[] }, b: { pos: number[] }) => {
        if (a.pos[0] !== b.pos[0]) return a.pos[0] - b.pos[0]
        return a.pos[1] - b.pos[1]
      })
      
      const chainNode = sortedNodes[0] // [100, 100]
      const httpNode = sortedNodes[1]   // [100, 250]
      const clientNode = sortedNodes[2] // [350, 180]
      const addressNode = sortedNodes[3] // [350, 350]
      const balanceNode = sortedNodes[4] // [600, 250]
      ...
      try {
        ...
        // 连接 Constant Address → getBalance
        addressNode.connect(0, balanceNode, 2)
    ...
    const executionResult = await page.evaluate(async () => {
      ...
      const balanceNode = sortedNodes[4]
